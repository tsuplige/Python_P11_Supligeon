...
@app.route("/board")
def pointBoard():
    return render_template('board.html', clubs=clubs), 200
...
def book(competition, club):
    try:
        ...
        if foundClub and foundCompetition:
            if verifyCompetitionDate(foundCompetition['date']):
...
def purchasePlaces():
    ...
    if (
        ...
        flash(
            f"The club does not have enough "
            f"points to order {placesRequired} seats."
        )
        return (
            render_template("welcome.html",
                            club=club,
    ...
    flash(f"Great-booking complete! You buy {placesRequired} places !")
